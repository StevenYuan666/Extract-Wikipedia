...
def extract_wikipedia_pages(file_path):
    ...
    with bz2.open(file_path, 'rt', encoding='utf-8') as reader:
        content = None
        for line in reader:
            line = line.strip()
            if line == '<page>':
                ...
                if text is None:
                    continue
                if "DIRECT" in text or "direct" in text: # Skip redirect pages
                    continue
                if "may refer to" in text.lower() and "disambiguation" in text.lower(): # Skip disambiguation pages
                    continue
                yield title, text
            else:
...
if __name__ == '__main__':
    cleaner = Cleaner()
    for title, text in extract_wikipedia_pages('enwiki-latest-pages-articles.xml.bz2'):
